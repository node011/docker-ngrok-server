...
./bin/ngrokd -tlsKey=/usr/local/certs/device.key -tlsCrt=/usr/local/certs/device.crt -domain="$DOMAIN" -httpAddr=":$HTTP_PORT" -httpsAddr=":$HTTPS_PORT" -tunnelAddr=":$TUNNEL_PORT"
